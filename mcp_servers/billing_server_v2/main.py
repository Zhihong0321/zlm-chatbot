...
def format_solar_impact(input_rm: float, morning_usage_pct: float, sunpeak_hour: float) -> str:
    ...
    return (
        "[OFFICIAL MCP RESULT]\n"
        "*** INSTRUCTION TO AGENT: Use this exact structure and figures. ***\n"
        "\n"
        f"1. We take User input of their TNB Bill monthly = RM {input_rm:.2f}\n"
        f"2. Which is translated into = {total_usage:.2f} kWh (derived from tariff rate)\n"
        f"3. We will suggest a {int(panel_qty + 0.99)} Panel Solar PV System ({panel_qty * panel_rating:.2f} kWp) to cover their usage\n"
        f"4. If we assume {morning_usage_pct}% day usage offset by Solar Generation (Self-Consumption: {consumed_solar:.2f} kWh)\n"
        f"5. Their bill would be reduced to RM {after_solar_rm:.2f}\n"
        f"6. And the excess Solar generation ({export_generation:.2f} kWh) will be exported to Grid for RM {export_income:.2f} credit\n"
        "\n"
