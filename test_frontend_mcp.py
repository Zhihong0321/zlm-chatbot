...
def test_backend_apis():
    ...
    try:
        ...
        if response.status_code == 200:
            print("✅ Z.ai Backend API: Working")
        else:
            print("❌ Z.ai Backend API: Not responding")
            return False
    except Exception as e:
        print(f"❌ Z.ai Backend API: Error - {e}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("❌ MCP Management API: Not responding")
            return False
    except Exception as e:
        print(f"❌ MCP Management API: Error - {e}")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if endpoint == "/servers":
                        ...
                        print(f"   ✅ {method} {endpoint} - {description} ({count} servers)")
                    elif endpoint == "/templates":
                        count = len(data) if isinstance(data, list) else 0
                        print(f"   ✅ {method} {endpoint} - {description} ({count} templates)")
                    else:
...
def demonstrate_frontend_integration():
    ...
    print("\n✅ Navigation and UI:")
    print("   • MCP Management menu item (🛠️ MCP)")
...
def setup_frontend_environment():
    ...
    print("   🛠️ Tool integration visualization")
...
<body>
    <div class="container">
        <h1>🛠️ MCP Frontend Integration Demo</h1>
    ...
    print("✅ Created frontend_mcp_demo.html - Open this file to see the demo")
...
def main():
    ...
    if not test_backend_apis():
        print("\n❌ Backend APIs not available. Start both servers:")
    ...
    if not test_mcp_system_status():
        print("\n⚠️ MCP system check failed, but continuing...")
    
    # Test endpoints
    if not test_mcp_endpoints():
        print("\n⚠️ Some endpoints failed, but core functionality should work")
    ...
    print("\n✨ What's Available:")
    print("1. 🛠️ Full MCP Management Dashboard")
